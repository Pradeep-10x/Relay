...
export function ProjectAnalyticsView({ projectId }: { projectId: string }) {
    ...
    return (
        <div className="px-6 py-6 w-full max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <div>
                <h2 className="text-xl font-bold tracking-tight text-white mb-1">Telemetry</h2>
                <p className="text-[13px] text-zinc-500">High-level metrics and issue progression states.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                {/* Metric 1 */}
                <div className="p-5 rounded-lg border border-zinc-800/80 bg-zinc-950 shadow-sm flex flex-col justify-between h-[130px]">
                    <div className="flex items-center gap-2 text-zinc-500">
                        <TrendingUp size={14} />
                        <span className="text-[10px] font-bold tracking-widest uppercase">Total Issues</span>
                    </div>
                    <div className="text-[32px] font-black text-white">{analytics.totalIssues}</div>
                </div>

                {/* Metric 2 */}
                <div className="p-5 rounded-lg border border-emerald-500/20 bg-emerald-500/5 shadow-sm flex flex-col justify-between h-[130px]">
                    <div className="flex items-center gap-2 text-emerald-500">
                        <CheckCircle size={14} />
                        <span className="text-[10px] font-bold tracking-widest uppercase">Completed</span>
                    </div>
                    <div className="text-[32px] font-black text-emerald-500">{analytics.completedIssues}</div>
                </div>

                {/* Metric 3 (Chart) */}
                <div className="md:col-span-2 p-5 rounded-lg border border-zinc-800/80 bg-zinc-950 shadow-sm flex items-center gap-8 h-[130px]">
                    <div className="relative w-[70px] h-[70px] flex-shrink-0">
                        <svg className="w-full h-full -rotate-90 drop-shadow-md">
                            <circle cx="35" cy="35" r={radius} fill="none" className="stroke-zinc-900" strokeWidth="6" />
                            <circle 
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <span className="text-[15px] font-black text-white">{ratePct}%</span>
                        </div>
                    </div>
                    <div>
                        <h4 className="text-[13px] font-bold text-zinc-200">Completion Velocity</h4>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
               {/* Issues Per State */}
               <div className="p-6 rounded-lg border border-zinc-800/80 bg-zinc-950 shadow-sm">
                   ...
                   <div className="space-y-5">
                       {analytics.issuesPerState?.map((item: any, i: number) => (
                           <div key={i} className="flex flex-col gap-2">
                               <div className="flex items-center justify-between text-[12px] font-semibold text-zinc-300 uppercase tracking-wider">
               ...
               <div className="p-6 rounded-lg border border-zinc-800/80 bg-zinc-950 shadow-sm">
                   ...
                   <div className="space-y-0">
                       {analytics.tasksPerUser?.map((item: any, i: number) => (
                           <div key={i} className="flex items-center justify-between py-2 border-b border-zinc-800/40 last:border-0 hover:bg-zinc-900/50 px-2 -mx-2 rounded-md transition-colors">
                               <span className="text-[12px] font-bold text-zinc-300">{item.assigneeId}</span>
                               <span className="text-[10px] font-bold font-mono text-zinc-500 bg-black border border-zinc-800 px-2 py-0.5 rounded">
                                   {item._count} issues
                               </span>
                           </div>
                       ))}
